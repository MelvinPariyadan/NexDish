...
def train(config=None):
    ...
    with wandb.init(project="NexDish", group="sweep", config=config):
        model_config = wandb.config

        train_loader, val_loader, test_loader, classes = get_datasets_and_loaders(model_config.batch_size, NUM_WORKERS, PIN_MEMORY)
        ...
        criterion = nn.CrossEntropyLoss() #CE for multi-class classification
...
def main():
  ...
  sweep_id = wandb.sweep(sweep_config, project="NexDish")
